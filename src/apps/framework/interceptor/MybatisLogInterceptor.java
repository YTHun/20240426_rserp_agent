...
import apps.project.batch.TaskBatch;
...
//	protected Logger	logger	= LogManager.getLogger(this.getClass());
	...
	// TODO Auto-generated method stub
        ...
        try{
	        ...
	        if(param == null){				// 파라미터가 아무것도 없을 경우
	            ...
	            if(param instanceof Integer || param instanceof Long || param instanceof Float || param instanceof Double){
		        	...
		        	for(ParameterMapping mapping : paramMapping){
		        	    ...
		        	    if (propValue.startsWith("__frch_item_")){
		        	    	String temp_value = boundSql.getParameterMappings().get(0).getProperty();
		        	    	
		        	    	//System.out.println("temp_value ====>"+temp_value+"    "+boundSql.getParameterMappings().size());
		        	    	CmMap reMap = (CmMap)param;
		        	    	
		        	    	
		        	    	ArrayList ar_temp = null;
		        	    	int chk_number = Integer.parseInt(temp_value.substring(12));
		        	    	//System.out.println("chk_number===>"+chk_number);
		        	    	int chk_int = 0;
		        	    	Iterator<String> keys = reMap.keySet().iterator();
		                    while( keys.hasNext() ){
		                        String key = keys.next();
		                        //System.out.println( String.format("키 : %s, 값 : %s  타입 : %s", key, reMap.get(key), reMap.get(key).getClass().getName() ) );
		                        
		                        if ( reMap.get(key) instanceof ArrayList ){
		                        
		                        	if ( chk_number == chk_int){
		                        		ar_temp = (ArrayList)reMap.get(key);
		                        		
		                        		for (int i=0 ; i <ar_temp.size() ; i++){
		                        			String f_temp = (String)ar_temp.get(i);
		                        			if(f_temp instanceof String){			// SQL의 ? 대신에 실제 값을 넣는다. 이때 String 일 경우는 '를 붙여야 하기땜에 별도 처리
		            		        	    	sql = sql.replaceFirst("\\?", "'" + f_temp + "'");
		            		        	    }else{
		            		        	    	if (value == null){
		            		        	    		sql = sql.replaceFirst("\\?", "");
		            		        	    	}else{
		            		        	    		sql = sql.replaceFirst("\\?", f_temp.toString());
		            		        	    	}
		            		        	    }
		        	...
		        	// logger.info("paramClass.getName() : {}", paramClass.getName());
		        	for(ParameterMapping mapping : paramMapping){
        	...
        	logger.info("MybatisLogInterceptor 오류 발생!"+e);
		}
